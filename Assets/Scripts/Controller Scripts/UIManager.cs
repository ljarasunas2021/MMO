...
namespace MMO.UI
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        public IEnumerator ToggleDialogue(Dialogue dialogue, NPCInteract nPC)
        {
            ...
            if (dialogue != null)
            {
                ...
                if (NPCDialogue != null) yield return StartCoroutine(PlayDialogue(NPCDialogue, nPC));
                else if (playerDialogue != null) yield return StartCoroutine(PlayDialogue(playerDialogue, nPC));
        ...
        private IEnumerator PlayDialogue(NPCDialogue dialogue, NPCInteract nPC)
        {
            ...
            {
                yield return StartCoroutine(PlayDialogue(dialogue.nextDialogue, nPC));
            }
            else if (!dialogue.nextDialogueIsNPC && (dialogue.options && dialogue.playerDialogueOptions.Length > 0) || (!dialogue.options && dialogue.playerDialogue != null))
            {
                ...
                {
                    yield return StartCoroutine(PlayDialogue(dialogue.playerDialogueOptions, nPC));
                }
                else if (!dialogue.options && dialogue.playerDialogue != null)
                {
                    yield return StartCoroutine(PlayDialogue(dialogue.playerDialogue, nPC));
                }
            }
            else
            {
                yield return StartCoroutine(ToggleDialogue(null, nPC));
            }
        }
