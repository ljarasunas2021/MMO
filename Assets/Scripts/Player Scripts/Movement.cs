...
namespace MMO.Player
{
    ...
    public class Movement : NetworkBehaviour
    {
        ...
        public void Move()
        {
            Move(new Vector2(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical")), UIManager.instance.canMove);           
        }

        public void Move(Vector2 inputVector, bool canMove)
        {
            ...
            SetLocomotionValues(inputVector, Input.GetButton("Sprint"), canMove);

            RotatePlayer(inputDir, Input.GetButton("Free Rotate Camera"), Input.mousePosition, canMove);

            CheckForJump(inputVector, Input.GetButton("Jump"), canMove);
        ...
        private void SetLocomotionValues(Vector2 input, bool leftShift, bool canMove)
        {
        ...
        private void RotatePlayer(Vector2 inputDir, bool leftControl, Vector2 mousePos, bool canMove)
        {
            if (!canMove) return;
        ...
        private void CheckForJump(Vector2 input, bool space, bool canMove)
        {
            if (!canMove) return;
        ...
        {
            animator.SetFloat(PlayerAnimParameters.locomotionBlend, 0);
            animator.SetFloat(PlayerAnimParameters.locomotionDir, 0);
            animator.SetFloat(PlayerAnimParameters.currentSpeedZ, 0);
            animator.SetFloat(PlayerAnimParameters.currentSpeedX, 0);
        }
    }
