...
namespace AnimFollow
{
    public class RagdollControl_AF : MonoBehaviour
    {
        ...
        [Range(2f, 26f)] [SerializeField] [Tooltip("The relative speed limit for a collision to make the character dose off")] private float graceSpeed = 8f;
        [Range(.1f, 1.7f)] [SerializeField] [Tooltip("The Limit of limbError that is allowed before the character doses off, given certain conditions")] private float noGhostLimit = .5f;
        [Range(0f, 1.2f)] [SerializeField] [Tooltip("makes the character glide free from an object if collision is not severe")] private float glideFree = .3f;

        [Header("Fall")]
        [Range(.5f, 4.5f)] [SerializeField] [Tooltip("Determines how fast the character loses control after colliding")] private float fallLerp = 1.5f;
        [Range(0f, .2f)] [SerializeField] [Tooltip("The torque immediately after collision")] private float residualTorque = 0f;
        [Range(0f, .2f)] [SerializeField] [Tooltip("The force immediately after collision")] private float residualForce = .1f;
        [Range(0f, 120f)] [SerializeField] [Tooltip("The joint torque immediately after collision")] private float residualJointTorque = 120f;
        [Range(10f, 50f)] [SerializeField] [Tooltip("Custom drag values during fall")] private float fallAngularDrag = 20f;

        [Header("Get Up")]
        [SerializeField] [Tooltip("These rigidbodies will get slippery during getup to avoid snagging")] private Transform[] IceOnGetup;
        [Range(10f, 170f)] [SerializeField] [Tooltip("Custom drag values during getup animations")] private float getupAngularDrag = 50f;
        [Range(5f, 85f)] [SerializeField] [Tooltip("Custom drag values during getup animations")] private float getupDrag = 25f;
        [Range(0f, .4f)] [SerializeField] [Tooltip("When ragdollRootBone goes below this speed the falling state is through and the get up starts")] private float settledSpeed = .2f;
        [Range(.04f, .48f)] [SerializeField] [Tooltip("Determines the initial regaining of strength after the character fallen to ease the ragdoll to the masters pose")] private float getupLerp1 = .15f;
        [Range(.5f, 6.5f)] [SerializeField] [Tooltip("Determines the regaining of strength during the later part of the get up state")] private float getupLerp2 = 2f;
        [Range(.05f, .65f)] [SerializeField] [Tooltip("A number that defines the degree of strength the ragdoll must reach before it is assumed to match the master pose and start the later part of the get up state")] private float wakeUpStrength = .2f;

        [Header("Contact")]
        [Range(0f, 10f)] [SerializeField] [Tooltip("The torque when in contact with other colliders")] private float contactTorque = 1f;
        [Range(0f, 10f)] [SerializeField] [Tooltip("The force when in contact with other colliders")] private float contactForce = 2f;
        [Range(0f, 50000f)] [SerializeField] [Tooltip("The joint torque when in contact with other colliders")] private float contactJointTorque = 1000f;
        [Range(0f, 700f)] [SerializeField] [Tooltip("Determines how fast the character loses strength when in contact")] private float toContactLerp = 70f;
        [Range(0f, 10f)] [SerializeField] [Tooltip("Determines how fast the character gains strength after freed from contact")] private float fromContactLerp = 1f;

        [Header("Torques / Forces")]
        [Range(0f, 100f)] [SerializeField] [Tooltip("The torque when not in contact with other colliders")] private float maxTorque = 100f;                // 
        [Range(0f, 100f)] [SerializeField] [Tooltip("The force when not in contact with other colliders")] private float maxForce = 100f;
        [Range(0f, 10000f)] [SerializeField] [Tooltip("The joint torque when not in contact with other colliders")] private float maxJointTorque = 10000f;
        [Range(0f, 1f)] [SerializeField] [Tooltip("The limit of error acceptable to consider the ragdoll to be matching the master. Is condition for going to normal operation after getting up")] private float maxErrorWhenMatching = .1f;

        private AnimFollow_AF animFollow;                   // The script that controlls the muscles of the ragdoll
        private PlayerMovement_AF playerMovement;           // To tell the character controller to no move when we are dosed off after a collision.
        ...
        private bool falling = false;            // Is in falling state
        private bool gettingUp = false;          // Is in getUp state
        private bool jointLimits = false;
        ...
        public void DoRagdollControl() // Needs to be synced with AnimFollow
        {
            ...
            if (numberOfCollisions > 0 && !(gettingUp || falling) && (collisionSpeed > graceSpeed || limbError.magnitude > noGhostLimit))
            {
                ...
                SetAnimFollowDrags(drag, fallAngularDrag);
